<template>
  <a-layout>
    <a-layout-header :style="{ position: 'fixed', zIndex: 1, width: '100%' }">
      ...
      >
        <a-menu-item v-for="forum in forums" :key="forum.forumId">{{ forum.forumName }}</a-menu-item>
        <a-button type="danger" style="position: absolute;top:15px;right: 340px;"
                  @click="putForumCategoryInfo">同步博客分类信息</a-button>
        <a-button type="danger" style="position: absolute;top:15px;right: 180px;"
                  @click="putArticleInfo">同步文章概要信息</a-button>
        <a-button type="danger" style="position: absolute;top:15px;right: 20px;"
                  @click="putArticleDetail">同步文章详细信息</a-button>
      </a-menu>
    </a-layout-header>
          ...
          <a-list item-layout="horizontal" :data-source="articleInfo">
            <template #renderItem="{ item }">
              <a-list-item>
                <a-list-item-meta>
                  <template #title>
                    <a @click="showModal(item.articleInfoId,item.title)"><p>标题：{{ item.title }}</p></a>
                  </template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const getCategoryFormForumId = (selectKey) => {
      ...
      axios.get(url + "/forumCategory/getForumCategoryByForumId/" + selectKey).then((resp) => {
        let content = resp.data.content;
    ...
    const getArticleInfo=(activeKey)=>{
      axios.get(url + "/article/selectByForumNavId/" + activeKey).then((resp) => {
        let content = resp.data.content;
        articleInfo.value = content;
      })
    };
    const getArticleDetail=(articleDetailId)=>{
      axios.get(url+"/article/selectDetailById/" + articleDetailId).then((resp)=>{
        let content = resp.data.content;
    ...
    const firstShowData= ()=>{
      axios.get(url + "/forum/getForums").then((resp) => {
        let content = resp.data.content;
        forums.value = content;
        axios.get(url + "/forumCategory/getForumCategoryByForumId/" + forums.value[0].forumId).then((resp) => {
          let content = resp.data.content;
          forumCategories.value = content;
          getArticleInfo(forumCategories.value[0].forumNavId)
        })
